import React, { useState, useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { SetLoading } from "../../Redux/loadersSlice";
import { message } from "antd";
import { getLoggedInUserName } from "../../utils/helper";
import { GetAllBloodBroupsInInventory } from "../../Apicall/dashboard";
import InventoryTable from "../../components/InventoryTable";
function Home() {
  const { currentUser } = useSelector((state) => state.users);
  const [bloodGroupsData = [], setBloodGroupsData] = useState([]);
  const dispatch = useDispatch();
  const getData = async () => {
    try {
      dispatch(SetLoading(true));
      const response = await GetAllBloodBroupsInInventory();
      dispatch(SetLoading(false));
      if (response.success) {
        setBloodGroupsData(response.data);
      } else {
        throw new Error(response.message);
      }
    } catch (error) {
      message.error(error.message);
      dispatch(SetLoading(false));
    }
  };

  useEffect(() => {
    getData();
  }, []);
  const colours = [
    "#2B3467",
    "#1A5F7A",
    "#B8621B",
    "#245953",
    "#2C3333",
    "#804674",
    "#A84448",
    "#635985",
  ];

  return (
    <div>
      <span className="text-white bg-red-600 text-2xl rounded p-1 w-full">
        Welcome {getLoggedInUserName(currentUser)}
        </span>
{currentUser.userType === "organization" && (
  <>
        <div className="grid grid-cols-4 gap-5 mb-5 mt-5">
       {bloodGroupsData.map((bloodGroup,index)=>{
          const color = colours[index];
        return (
          <div className={` p-5 flex justify-between text-white rounded items-center`} style={{backgroundColor:color}}>
           <h1 className="text-5xl uppercase">
            {bloodGroup.bloodGroup}
            </h1> 
            <div className="flex flex-col justify-between gap-2">
              <div className="flex justify-between gap-5 mb-1">
                <span>Total In</span>
                <span>{bloodGroup.totalIn}ML</span>
              </div>
              <div className="flex justify-between gap-5 mb-1">
                <span>Total Out</span>
                <span>{bloodGroup.totalOut}ML</span>
              </div>
              <div className="flex justify-between gap-5 mb-1">
                <span>Available</span>
                <span>{bloodGroup.available}ML</span>
              </div>
            </div>
          
          </div>
        )
       })}
      </div>
      <h5 className="text-xl text-green-600 font-semibold ms-2 mb-5 rounded p-1">
            Your Recent Inventory
          </h5>
      <InventoryTable  filters={{
              organization: currentUser._id,
            }}
            limit={6}
            userType={currentUser.userType}/>
  </>
)}

{currentUser.userType === "donor" && (
  <div>
   <h5 className="text-xl text-green-600 font-semibold mt-5 mb-5 rounded p-1">
            Your Recent Donations
          </h5>
      <InventoryTable  filters={{
              donor: currentUser._id,
            }}
            limit={6}
            userType={currentUser.userType}/>
  
  </div>
)}

{currentUser.userType === "hospital" && (
  <div>
   <h5 className="text-xl text-green-600 font-semibold mt-5 mb-5 rounded p-1">
            Your Recent Requests / Consumptions
          </h5>
      <InventoryTable  filters={{
              hospital: currentUser._id,
            }}
            limit={6}
            userType={currentUser.userType}/>
  
  </div>
)}
    </div>
  );
}

export default Home;
